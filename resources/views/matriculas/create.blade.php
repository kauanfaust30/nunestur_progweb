...
<div class="section-card" style="max-width:560px">
    <div class="section-head">
        <h2><i class="bi bi-clipboard-plus" style="margin-right:6px;color:var(--orange)"></i>Dados da matrícula</h2>
    </div>
        ...
        <form method="POST" action="/matriculas">
            ...
            <div style="margin-bottom:1rem">
                ...
                <select name="aluno_id" class="form-control" required>
                    ...
                    @foreach($alunos as $a)
                        <option value="{{ $a->id }}" {{ old('aluno_id') == $a->id ? 'selected' : '' }}>
                            {{ $a->nome }}
                        </option>
                    @endforeach
                </select>
            ...
            <div style="margin-bottom:1rem">
                ...
                <select name="instituicao_id" class="form-control" required>
                    ...
                    @foreach($instituicoes as $i)
                        <option value="{{ $i->id }}" {{ old('instituicao_id') == $i->id ? 'selected' : '' }}>
                            {{ $i->nome }}
                        </option>
                    @endforeach
                </select>
            ...
            <div style="margin-bottom:1.5rem">
                ...
                <input
                    ...
                    value="{{ old('dias_semana') }}">
            </div>

            <div style="display:flex; gap:8px;">
                <button type="submit" class="btn-orange">
                    <i class="bi bi-check-lg"></i> Salvar matrícula
                </button>
